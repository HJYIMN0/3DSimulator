...
[CreateAssetMenu(fileName = "State_Customer_SitDownTable", menuName = "States/Customer/SitDownTable")]

public class State_Customer_SitDownTable : StateEntity_SO
{
    [Header("Setting")]
    ...
    public override void Enter(Controller_Entity ce)
    {
        ...
        ce.CurrentItem.transform.SetParent(ce.MyChair.transform);
        ce.CurrentItem.transform.position = iKPosition.LookTarget.position;
    }
    ...
    public override void StateUpdate(Controller_Entity ce)
    {
        ...
        else if (time >= PreEnterRotationTime && time < (PreEnterRotationTime + iKPosition.AnimationEnter.length))
        {
            ...
            {
                if (ce.Animator_Generic != null && iKPosition.AnimationEnter != null) ce.Animator_Generic.PlayLoopAction(iKPosition.AnimationEnter, true, 0);
            }
        ...
        else if (time >= (PreEnterRotationTime + iKPosition.AnimationEnter.length))
        {
            ...
            if (loopTime <= Time.deltaTime)
            {
                ...
                if (ce.Animator_Generic != null && iKPosition.AnimationLoop != null) ce.Animator_Generic.PlayLoopAction(iKPosition.AnimationLoop, true, 0);
